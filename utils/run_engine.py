...
def train(model, loader, optimizer, loss_fn, device,dispersive_cfg):
    model.train()
    ...
    for i, ((x), (y1, y2)) in enumerate(tqdm(loader, desc="Training", total=len(loader))):
        ...
        optimizer.zero_grad()
        # 修改：模型 forward 返回 dispersive_loss
        mask_pred, fg_pred, bg_pred, uc_pred,dispersive_loss = model(x)
        ...
        if dispersive_loss is not None and dispersive_cfg is not None:
            # 兼容 dispersive_cfg 可能不同 key
            weight = dispersive_cfg.get('dispersive_loss_weight', dispersive_cfg.get('weight', 0.6))
            loss = loss_mask + beta1 * loss_fg + beta2 * loss_bg + loss_comp + weight * dispersive_loss
