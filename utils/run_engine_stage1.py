...
class DATASET(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if image is None:
            raise FileNotFoundError(f"图像读取失败，路径无效或文件损坏: {image_path}")
        if mask is None:
            raise FileNotFoundError(f"掩码读取失败，路径无效或文件损坏: {mask_path}")
...
def train(model, loader, optimizer, loss_fn, device, dispersive_cfg,consistency_loss_fn=BinaryConsistencyLoss()):
    model.train()
    ...
    for i, ((x), (y1, y2)) in enumerate(tqdm(loader, desc="Training", total=len(loader))):
        ...
        mask_pred ,dispersive_loss= model(x)

        x_aug = augmentations(x)
        x_aug = x_aug.to(device, dtype=torch.float32)
        #原本不加入色散损失：mask_pred_aug = model(x_aug)
        mask_pred_aug, dispersive_loss_aug = model(x_aug)  # ✅ 解包
        ...
        # 加 dispersive loss
        if dispersive_cfg is not None:
            weight = dispersive_cfg.get('weight', 0.2)
        if dispersive_loss is not None:
            loss = loss + weight * dispersive_loss
        if dispersive_loss_aug is not None:
            loss = loss + weight * dispersive_loss_aug
...
def evaluate(model, loader, loss_fn, device, dispersive_cfg):
    model.eval()
    ...
    with torch.no_grad():
        for i, ((x), (y1, y2)) in enumerate(tqdm(loader, desc="Evaluation", total=len(loader))):
            ...
            loss = loss_mask
            if dispersive_loss is not None and dispersive_cfg is not None:
                weight = dispersive_cfg.get('weight', 0.2)
                loss = loss + weight * dispersive_loss
            epoch_loss += loss.item()
